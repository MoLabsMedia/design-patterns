class StackCalculator
  parseString: ( string ) =>
    @stack = {}
    for _, token in ipairs( string )\split /\s+/
      @parseToken( token )
...
try
  ...
  print err -- => "Can't operate on a stack without at least 2 items"

try
  calc\parseString( "5 5 5 -" )
catch err
  print err -- => "Not enough operators: numbers left over"

try
  calc\parseString( "5 5 5 foo" )
catch err
  print err -- => "Unrecognized operator: foo"
